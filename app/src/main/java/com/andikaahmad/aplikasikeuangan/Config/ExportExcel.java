...
public class ExportExcel {
    ...
    public void create2() throws IOException, WriteException {
        ...
        Cursor c = db.select(query) ;
        if (db.countRows(query) > 0 && kolom.length == kolom2.length){
            File file = new File(path + ".xls") ;
            wbSettings = new WorkbookSettings();
            wbSettings.setLocale(new Locale("en", "EN"));
    ...
    public void setHeader(int JumlahKolom){
        try {
            ...
            mergeCells(getValue(q,"nama"),0,JumlahKolom,row++,true);
            mergeCells(getValue(q,"alamat"),0,JumlahKolom,row++,true);
            mergeCells(getValue(q,"notelp"),0,JumlahKolom,row++,true);
        }catch (Exception e){
    ...
    public static String removeE(String value){
        double hasil = strToDouble(value) ;
